...
BEGIN
    ...
    LOOP
        EXECUTE format('CREATE POLICY "Admins have full access to %I" ON public.%I FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = true))', t, t);
    END LOOP;
END $$;
